...
export class BoardScene extends Phaser.Scene {
    ...
    WebFont.load({
      ...
      active: function() {
        for(let i=1; i<= courses.length; i++){
          add
          .text(160, (gameHeight-i*32) - (i*128), courses[i-1].name, {
  ...
  addModules(platformIndex){
    let courses = this.service.courses[platformIndex-1];
    
    // if(platformIndex%2 == 0){ courseModule = courseModule.reverse();  }

    // this.actorModule = 0;
    let batchCount = 0;
    for(let count = 1; count < courses.modules.length + 1; count++){
      ...
      let moduleStatus = Number(moduleInfoSplit[1]);
      ...
      if(moduleStatus == 0){
        if(moduleType == "forum"){
          if(platformIndex%2 == 0){
            module = this.physics.add.sprite(120 + (16-count) * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "forum").setScale(0.8, 0.8);
          } else {
            module = this.physics.add.sprite(120 + count * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "forum").setScale(0.8, 0.8);
          }
          
        // module.anims.play("lessionRotate", true);
        } else if(moduleType == "questionnaire"){
          if(platformIndex%2 == 0){
            module = this.physics.add.sprite(120 + (16-count) * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "questionnaire").setScale(0.8, 0.8);
          } else {
            module = this.physics.add.sprite(120 + count * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "questionnaire").setScale(0.8, 0.8);
          }
          
          //module.anims.play("lessionRotate", true);
        } else if(moduleType == "quiz") {
          if(platformIndex%2 == 0){
            module = this.physics.add.sprite(120 + (16-count) * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "quiz").setScale(0.8, 0.8);
          } else {
            module = this.physics.add.sprite(120 + count * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "quiz").setScale(0.8, 0.8);  
          }
          
        // module.anims.play("lessionRotate", true);
        }else if(moduleType == "label") {
          if(platformIndex%2 == 0){
            module = this.physics.add.sprite(120 + (16-count) * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "label").setScale(0.8, 0.8);
          } else {
            module = this.physics.add.sprite(120 + count * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "label").setScale(0.8, 0.8);
          }
          
        // module.anims.play("lessionRotate", true);
        } else {
          if(platformIndex%2 == 0){
            module = this.physics.add.sprite(120 + (16 - count) * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "lessionSprite").setScale(0.8, 0.8);
          } else {
            module = this.physics.add.sprite(120 + count * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "lessionSprite").setScale(0.8, 0.8);   
          }
        ...
        if(moduleType === "lesson"){
          if(platformIndex%2 == 0){
            module = this.physics.add.sprite(120 + (16 - count) * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "video").setScale(0.8, 0.8);
          } else {
            module = this.physics.add.sprite(120 + count * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "video").setScale(0.8, 0.8);
          }
          
        } else {
          if(platformIndex%2 == 0){
            module = this.physics.add.sprite(120 + (16 - count) * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "complete").setScale(0.8, 0.8);
          } else {
            module = this.physics.add.sprite(120 + count * 64, (this.gameHeight-platformIndex*32) - platformIndex*128, "complete").setScale(0.8, 0.8);  
          }
          batchCount++;
          this.add.sprite(this.gameWidth - batchCount*64 - 64, (this.gameHeight-platformIndex*32) - (platformIndex*128) + 32,"badgeSprite",batchCount).setScale(0.3,0.3).setDepth(5);
        }
  ...
  service(){
    ...
    this.load.json('data', 'http://localhost:3001/game/studentinfo?'+this.studentId);
    // this.load.json('data', 'http://35.178.169.194/moodle/phaser/index.html?id='+this.studentId);
